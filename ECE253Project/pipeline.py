...
# ============================================================
# MULTITASK MODEL (MATCHES TRAINING)
# ============================================================
...
# If your PyTorch is new enough, you can do weights_only=True
...
# ============================================================
# FOG MODULE UTILITIES
# ============================================================
def defog_image_clahe(foggy_rgb):
...
def run_fog_module(input_rgb, gt_rgb=None, show_plots=True):
    ...
    print(f"[FogModule] Fog detected (fog_prob={fog_prob:.3f}) → defogging.")

    clahe_img = defog_image_clahe(input_rgb)
    hist_img  = equalize_histogram_rgb(input_rgb)

    gray_in    = to_gray(input_rgb)
    gray_clahe = to_gray(clahe_img)
    gray_hist  = to_gray(hist_img)

    ten_in    = tenengrad_sharpness(gray_in)
    ten_clahe = tenengrad_sharpness(gray_clahe)
    ten_hist  = tenengrad_sharpness(gray_hist)

    br_in    = brightness_mean(gray_in)
    br_clahe = brightness_mean(gray_clahe)
    br_hist  = brightness_mean(gray_hist)

    print("\n[FogModule] Tenengrad / Brightness:")
    print(f"  Input : Ten={ten_in:.1f},  Bright={br_in:.1f}")
    print(f"  CLAHE: Ten={ten_clahe:.1f}, Bright={br_clahe:.1f}")
    ...
    if show_plots:
        plt.figure(figsize=(15, 5))
        plt.subplot(1, 3, 1); plt.imshow(input_rgb);  plt.title("Input (possibly foggy)"); plt.axis("off")
        plt.subplot(1, 3, 2); plt.imshow(clahe_img); plt.title("CLAHE defog (chosen)");   plt.axis("off")
        plt.subplot(1, 3, 3); plt.imshow(hist_img);  plt.title("HistEq defog (debug)");    plt.axis("off")
        plt.tight_layout()
        plt.show()
...
def plot_aggregate_metrics(agg_metrics):
    ...
    # Helper for nice labeled bar plots with error bars
    def plot_metric_group(title, metrics_in_group, pretty_names, ylabel=None):
        ...
        if len(filtered) == 0:
            return  # nothing to plot in this group

        n_metrics = len(filtered)
        x = np.arange(len(variants))
        width = 0.15 if n_metrics > 3 else 0.2  # bar width
    ...
    plot_metric_group(
        title="Sharpness & Brightness Metrics",
        metrics_in_group=["tenengrad", "brightness"],
        pretty_names=["Tenengrad Sharpness", "Mean Brightness"],
        ylabel="Value",
    )
...
def main():
    ...
    print("\n=== Unified Pipeline: Demo (user-controlled) + Metrics on Test Set ===")
    ...
    # Prefer rows whose filepath basename starts with CLEAN_.
    clean_candidates = []
    for path in df_test["filepath"]:
        base = os.path.basename(path)
        if base.startswith("CLEAN_"):
            clean_candidates.append(path)

    if len(clean_candidates) > 0:
        CLEAN_IMAGE_PATH = np.random.choice(clean_candidates)
    else:
        # fallback: any random test image (we'll treat it as clean for synthetic demo)
        CLEAN_IMAGE_PATH = df_test.sample(1, random_state=42)["filepath"].iloc[0]

    NATURAL_IMAGE_PATH = CLEAN_IMAGE_PATH  # you can customize if needed
    ...
    # 2) Demo blur module
    blur_demo, gt_after_blur_demo, meta_blur_demo, _ = run_blur_module(
        degraded_demo, gt_demo, show_plots=True
    )

    # 3) Demo fog module
    fog_demo, gt_after_fog_demo, meta_fog_demo, _ = run_fog_module(
        blur_demo, gt_after_blur_demo, show_plots=True
    )

    # 4) Demo low-light module
    low_demo, gt_after_low_demo, meta_low_demo, _ = run_lowlight_module(
        fog_demo, gt_after_fog_demo, show_plots=True
    )

    # 5) Summary plots for the demo
    plt.figure(figsize=(20, 4))

    cols = 4 if gt_demo is not None else 3
    idx = 1
    ...
    plt.title(f"Demo: After Generator (code={meta_gen_demo['code']})")
    plt.axis("off")

    plt.subplot(1, cols, idx); idx += 1
    plt.imshow(fog_demo)
    plt.title("Demo: After Blur+Fog")
    plt.axis("off")

    plt.subplot(1, cols, idx)
    plt.imshow(low_demo)
    plt.title("Demo: After LowLight (Retinex)")
    plt.axis("off")
    ...
    variants = ["degraded", "after_blur", "after_fog", "after_lowlight"]
    ...
    for idx_row, row in sampled_rows.iterrows():
        ...
        low_out,  gt3, _, _ = run_lowlight_module(fog_out, gt2, show_plots=False)

        variants_imgs = {
            "degraded":       deg_rgb,
            "after_blur":     blur_out,
            "after_fog":      fog_out,
            "after_lowlight": low_out,
        }
